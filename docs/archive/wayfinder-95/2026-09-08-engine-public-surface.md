...
2026-09-08 用户确认沿用 `10f3db4`：ERROR_TYPES 留在 internal，公开消费者使用 TypedError['type']。仓内数组仍供协议类型推导、契约导出脚本及版本失败回归使用，未发现公开主入口导入者。#113 验收正文已对齐并关闭；`009e11b` 的远端 CI 全部通过。#114 接续构建，#115/#116 仍按依赖顺序推进。

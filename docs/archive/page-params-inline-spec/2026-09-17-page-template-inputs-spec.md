...
2026-09-20 更新：本文参数声明形状由[分组参数与多时间输入方案](../../plan/2026-09-20-grouped-page-params.md)替代，后者以 6.6 实现。下文数组/value 设计保留为历史方案，不再作为新实现依据。
状态：产品边界已确认；以下同构 Schema 方案按用户最新纠正提出，待实现及审查。
本文替代本文件此前的独立 PageTemplate / inputs / targets 方案。没有新增模板包装、第二份页面 Schema 或资产附属映射。

执行拆分见 [实施计划](tasks.md)，验收追溯见 [验收用例](acceptance.md)。计划尚未执行。

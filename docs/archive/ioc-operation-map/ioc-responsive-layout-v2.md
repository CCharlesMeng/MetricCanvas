...
现有 `docs/archive/ioc-operation-map/ioc-1980-width/` 缺少 `sdd-task-frontend` 要求的 `story-delta-spec.md`（SC/BR/GWT）和 codespec schema 指令，因此本文没有改写原 Story 的 `tasks.md` / `alpha-tests.md`，也没有伪造新的 `UNVERIFIED` 追溯账本。本次实施是针对已确认回归的布局重构，不依赖外部设计源还原，按仓库约定使用普通 TDD 验证链收口。

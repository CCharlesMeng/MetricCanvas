...
> 阅读 `docs/archive/ioc-operation-map/ioc-project-overview-fidelity-refinement.md`，为该改进创建新的 SDD Story，并使用 `sdd-task-frontend` 生成正式任务和验收账本；确认后再使用 `sdd-dev-frontend` 实施。不得修改已验收的 IOC-S2 Story。保留 `CompositeCard`，不新增大一统卡片组件。

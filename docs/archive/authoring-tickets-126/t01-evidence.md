...
读取前的永久 seen 去重已改为按身份隔离的 pending/accepted 记录：进行中同 ID 只读一次，成功重复不重放；失败、非法文档、引用不匹配、作用域过期均释放本次记录。每个请求拥有独立记录身份，旧请求清理不会删除新请求记录。工作台单列 identity 与 epoch：手工编辑令旧读失效，但不会使已成功 ID 因编辑再次覆盖手工内容。

SDK 固定为每个 document 一个资源 URL/版本，重复挂载同版本共用加载，失败同版本可重试；同页面切换版本明确拒绝并提示刷新。浏览器补验 v1→请求v2被拒绝→恢复v1成功；整页重新加载后v2成功。因此没有缓存v1却实际挂v2的混用。

新增回归后3文件22测试通过；svelte-check仍0错0警告；浏览器脚本再次通过。上文256 ID限制只适用于成功/进行中记录，不再阻断失败重试。对应测试涵盖首次失败后同ID成功、进行中重复、身份改变重读、旧请求清理隔离及同页面版本拒绝。

第二次复核补充：新通知或卸载取消旧读取时，同步释放该请求自己的 pending 记录，不等待忽略 abort 的适配器 settle；accepted 记录不清除。新增不响应 abort 的 A→B→A 用例，第三次通知确实重读，旧 A/B 迟到均不覆盖且不清新记录。相关回归更新为3文件23测试全部通过。

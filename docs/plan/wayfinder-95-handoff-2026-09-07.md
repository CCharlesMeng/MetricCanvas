...
[#106 Relay Chat 接口的待确认事项](https://github.com/CCharlesMeng/MetricCanvas/issues/106) 没有原生 blocker，标签是 `wayfinder:task`，但它是**提供方问答**，不是开发票。

- 已有接口原文在 [#106 评论](https://github.com/CCharlesMeng/MetricCanvas/issues/106#issuecomment-5567401672)，不需要用户重新贴。
- 九项包括地址/传输协议、订阅认证、事件结构、traceId、错误/重试、停止语义、确认/取消上行、浏览器请求头支持、会话/检查点读取。
- 对接时一并核实 #107 所需的受控 Skill 路由，以及 #108 所需的 Page Artifact Adapter/完整产物读取能力；这两项已有独立问题归属，不要遗漏或凭猜测认定支持。
- 现在可以准备提问清单、整理已有回答与来源；用户或获明确授权的对接者取得提供方回答。不得在未获发送授权时替用户联系他人。
- **“没有”也是有效答案。** 缺答案不能当作接口不支持；文档没有写到也不能等同于服务实际没有。

**启动语句：**

> 阅读本 handoff，继续地图 #95，只推进 #106。读取票内接口原文，整理尚缺的事实和对提供方的提问清单；纳入 #107 的 Skill 路由与 #108 的产物/恢复能力问题。已取得的回答逐条记录来源和日期，未知项保持未知，不猜协议，不实现适配器，也不自行发送消息给提供方。
...
- Java/Relay/Python 的现行基线是 ADR-0060～0064。**浏览器不直连旧 Relay Chat 接口，改经中台公共“发送消息服务”**是地图新增已决输入；协议事实仍待 #106，不能把旧调查中的 WebSocket 当作新服务协议。

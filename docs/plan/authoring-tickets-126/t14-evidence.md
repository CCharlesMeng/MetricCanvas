...
统一languageBlocked资格用于readSavedDraft、acceptSavedDraft及requireSynchronizedRef：现有工作副本初始保护仍pending时拒绝，owner与当前身份不符时拒绝，dispose后拒绝；语言精确读在不具资格时不调用外部端口，异步读返回后再次核对范围和资格。空画布没有工作副本/ref时仍允许创建通知的精确读与接收，不误要求所有创建先有修订。

新增2项回归：挂起初始storage.write时loading=false/queue=0仍拒绝读取和接收（零外部read），保护完成后允许；已保护零队列切换身份后读取/接收/引用均拒绝。覆盖空画布创建及dispose后的引用拒绝。补正后四文件53项通过、tsc和diff检查通过；T14恢复浏览器及T01/T02/T13对话工作台浏览器复验通过。上方全量1009项为ef0597d3实施基线记录，本次为针对门禁补正的最小充分复验。

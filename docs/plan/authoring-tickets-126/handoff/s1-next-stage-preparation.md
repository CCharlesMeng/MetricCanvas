...
S5 只读复核指出必须单列的缺口：现有 `captureScope` 在通知到达时取值，只隔离已发起读取之后的作用域变化；“旧轮取消→新轮开始→旧 draftId 首次通知迟到”会捕获新作用域，现有迟到读取测试不覆盖它。T01 单字段事件无法自行证明轮归属；#146 的可信读取/协调边界必须取得程序通道中的操作与来源基线关联，验证当前接收资格。真实关联未证实前保持生产精确读取不可用；替身也必须显式构造上述反例，不能通过扩展事件 JSON 或推测 ID 绕过。

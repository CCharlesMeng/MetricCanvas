...
- [ADR-0069](adr/0069-local-boundary-substitutes-and-host-owned-credentials.md) 明确所有数据请求的请求头与相关 Cookie 均由宿主决定，覆盖 DQE、页面资产和公共 Chat 的宿主请求。本仓不新增开发身份适配器或用户切换器，不自行补默认用户或覆盖宿主凭据；Cookie 由浏览器按宿主选择的请求策略处理。该边界不免除服务端必填字段与权限校验，具体接口与 CORS 接线由 #101/#105/#106 对账。

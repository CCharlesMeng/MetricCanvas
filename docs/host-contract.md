...
响应式断点以统一运行时挂载容器的 CSS 宽度为准。2560×1440 屏幕在 150% 系统缩放下，逻辑屏幕约为 1707×960；浏览器工具栏、门户导航和侧栏还会减少页面的实际可用空间。

`RuntimeView` 声明具名 `mc-runtime` 容器，页面外框、内容分区及组件的整页响应式规则共同消费它：1050px 以下收紧内距，760px 以下回流为单列并调整页头。指标卡内部按自身宽度生效的容器查询继续独立工作。新增整页断点应使用这个具名容器，避免读取窗口宽度或误匹配组件内部的容器。

宿主应允许挂载容器收缩（例如 Flex/Grid 内容槽设置 `min-width: 0`），并提供正常的纵向滚动。不要用物理屏幕分辨率判断布局，也不要用 CSS `zoom` 或反向缩放抵消系统缩放。报表保持定宽上限、在空间不足时回流；纵向长报表允许滚动阅读。

验证入口：`pnpm --filter @metriccanvas/embed exec playwright test report-responsive.spec.ts`（先运行该包的 `build`）。用例模拟 1707px 视口与 1.5 像素比，覆盖独立打开、侧栏压缩、窄容器和恢复宽屏，并比较相同宽度在窗口与嵌入容器中的布局。它是浏览器几何回归，不替代目标操作系统上的实际显示验收。

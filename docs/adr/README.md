# ADR 基线：82 份决策记录的当前生效结论

`docs/adr/` 现有 82 份 ADR(0001–0082)。多份后出 ADR 部分或全部取代了早前 ADR 的前提,单独阅读任意一份都无法确认它在今天是否仍然生效。本文件按主题聚合这些 ADR 追踪到的**当前生效结论**,不是新决策,也不改写或删除任何原文。
...
[ADR-0082](./0082-explicit-business-sections-in-platform-authoring.md)：完整报告通过页面结构计划显式组织章节，场景参考与章节组合提供可调整的默认结构；允许跨口径同章与一源多组件，以可见说明保留口径。ADR-0055 的自动口径分区保留给普通问数和兼容快速装配。计划不进入渲染协议；首期不包含自动分析结论。

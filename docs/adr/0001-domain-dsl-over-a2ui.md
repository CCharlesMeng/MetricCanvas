...
> ADR-0014 保留自研领域 DSL 决策，但将其数据语义从指标/维度结构化查询修订为查询执行引用、参数契约与结果字段契约。
...
- 需要自建规格 schema 与运行时,schema 演进必须自我约束:规格不表达像素级样式、不表达任意计算逻辑(复杂计算下沉数据服务),防止 DSL 膨胀为"用 JSON 写的编程语言"。

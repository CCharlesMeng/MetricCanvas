...
页面元数据是统一运行时消费的声明式JSON文档。当前作者协议为 **6.2**，公开读取兼容6.0/6.1/6.2。结构事实由 `packages/page/src/schema/` 单向导出；未声明属性不被接受。
...
新文档写6.2和layout；6.0的layoutForm在输入边界兼容读取，双字段同时出现拒绝。规范化保留已支持版本的能力边界，旧文档迁移另存新修订；先核验历史原文hash，再规范化。详见[布局迁移](docs/page-metadata/layout-migration.md)。

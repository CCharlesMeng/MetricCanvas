...
`shell` 是分区外观的唯一真源，可选，封闭三档且命名表现中性：

| 值 | 外观 |
|---|---|
| `plain` | 无外壳，组件完全自带外观（报告头、摘要指标卡行、`variant: "heading"` 的分隔标题） |
| `panel` | 渐变章节面板 + 居中图标标题 + 内层白底承载网格 |
| `card` | 白色小节卡片 + 左对齐小标题 |

缺省保持通用看板外观：白色分区 + 带边框的组件单元格。统一运行时不根据组件组合或子组件 `props.variant` 推断分区外观；三档外壳下组件单元格一律无镶边，表面由组件自带。

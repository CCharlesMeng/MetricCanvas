...
- [维度参数](#维度参数)：单值/多值、查询原位引用和筛选初值。
- [时间参数](#时间参数)：明确区间与报告基准期、窗口、指标口径、无数据及初始化约束。
- 场景一至四保留完整的生成、提取模板、赋值运行和召回流程。
...
维度参数确定“看哪个业务范围”，例如代表处或地区。单值使用非空字符串，多值声明 `multiple: true` 并使用无重复、非空的字符串数组；取值必须由数据服务契约确认。

```json
{
  "params": [{ "id": "region", "type": "dimension", "value": "中国区" }]
}
```

查询在 `filter.dims` 中用 `{ "dim_name": "区域", "dim_value_list": { "param": "region" } }` 消费参数，解析时单值也转为数组。文本用 `{ "param": "region" }` 显示同一实际值。参数必须有消费者，声明本身不会自动影响全页。筛选初值见[参数与页内筛选](#7-参数与页内筛选)。
...
| 类型 | 回答的问题 | 输入与查询 |
|---|---|---|
| `timeRange` | 明确读取哪段时间？ | `value: {start:"2026-01",end:"2026-06",granularity:"month"}`，查询双端引用同一参数的 `part:start/end`，不带窗口 |
| `time` | 以哪一期为基准？ | `value: "2026-06"`、`granularity: "month"`，各查询双端引用同一参数并声明相同 `window` |

### 报告基准期、查询时间窗口与指标口径

报告基准期决定本次看哪一期；查询窗口决定读取哪些期间；指标口径决定一个值代表什么。窗口不会自动计算同比、环比或改写年累计口径。时间输入精度是 `month/date`，查询粒度分别为 `month/day`，不隐式转换。
...
通过 `params[].value` 或程序输入 `suppliedValues` 赋值；显式输入优先于保存值。缺值或非法输入应先处理解析问题，不能回退旧值后继续查询。`timeRange` 尚无 URL 编码协议。模板不携带本次值，填值文档与执行副本分离，运行不会自动保存页面资产。

查询起止必须引用同一时间参数，分别声明 `part: "start"` 和 `part: "end"`；`time` 的双端窗口必须一致，`timeRange` 不声明窗口。时间参数不能与时间筛选器同时控制同一查询，也不另保留一套静态起止。参数化页面不使用旧查询初始行兜底。

完整输入优先级、错误分类和日期边界见[运行前必须检查](#运行前必须检查)，完整赋值用法见[本次输入与调用](#本次输入与调用)。

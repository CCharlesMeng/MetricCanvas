...
仓库文件仍是可评审的页面资产，平台通过 PostgreSQL 保存不可变页面修订并向统一运行时提供已发布版本。当前工作台是最小 AI 搭建闭环，不是通用拖拽式可视化搭建器，也不包含 A2UI 第二渲染通道。真实数据服务鉴权、SSO 和部署由接入环境提供；仓库内的仿真头仅用于本地开发，不能当作生产配置。
...
VITE_DATA_GATEWAY=sim pnpm dev
```

如需改地址：

```bash
VITE_DATA_GATEWAY=sim VITE_DATA_SERVICE_URL=http://localhost:18226 pnpm dev
```
...
- **启动**：本地协议验证运行 `pnpm sim`，另一个终端运行 `VITE_DATA_GATEWAY=sim pnpm dev`。

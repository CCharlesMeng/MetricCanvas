...
import { syncCatalog, DEFAULT_AGGREGATIONS } from '../src/sync-catalog';
...
describe('sync-catalog 拼装器(services/list + __type 内省 + MetricBaseInfo)', () => {
  it('三类请求打到正确端点,GraphQL 查询体包装为 {apiQuery, isTest}', async () => {
    ...
    expect(queries.some((q) => q.includes('__type(name:"P001_ADS_T_IOC_SPD_METRIC_ACC_D")'))).toBe(true);
    expect(queries.some((q) => q.includes('__type(name:"F34_ioc")'))).toBe(true);
    expect(queries.some((q) => q.includes('MetricBaseInfo'))).toBe(true);
  });
  ...
  it('指标来自 MetricBaseInfo,维度来自内省字段(剔除度量列),快照按 code 排序、diff 可读', async () => {
    ...
    // 后端尚未提供"可用维度/可用聚合"(共建中):初版按宽松默认填充
    for (const metric of snapshot.metrics) {
      expect(metric.availableDimensions).toEqual(expectedDimensions);
      expect(metric.availableAggregations).toEqual(DEFAULT_AGGREGATIONS);
    }

...
/**
 * 行归一化问题 → 数据网关错误。issue 上挂着的原始行与原始值不进入
 * 错误 detail:只保留行号、字段名、类型名与数量等结构化事实(issue #47)。
 */
  ...
  switch (issue.code) {
    case 'ROWS_NOT_ARRAY':
      return new DqeGatewayError(
        'DQE_ROW_CONTRACT_ERROR',
        'DQE data 必须是数组',
        { actualType: describeType(issue.actual) }
      );
    case 'ROW_NOT_OBJECT':
      return new DqeGatewayError(
        'DQE_ROW_CONTRACT_ERROR',
        `DQE data[${issue.rowIndex}] 必须是对象`,
        { rowIndex: issue.rowIndex, actualType: describeType(issue.actual) }
      );
    ...
    case 'FIELD_TYPE_MISMATCH':
      return new DqeGatewayError(
        ...
        { fieldId: issue.fieldId, valueType: describeType(issue.value) }
      );
    ...
    case 'DETAIL_ITEM_NOT_OBJECT':
      return new DqeGatewayError(
        ...
        `嵌套明细字段 ${issue.queryField}[${issue.itemIndex}] 必须是对象`,
        {
          ...
          valueType: describeType(issue.value)
        }
      );

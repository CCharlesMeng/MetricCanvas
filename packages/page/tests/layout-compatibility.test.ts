...
describe('6.1 layout 兼容公开边界', () => {
  for (const schemaVersion of ['6.5']) {
    for (const form of ['report', 'dashboard']) {
      it(`${schemaVersion} layoutForm ${form} 保持布局且只写新版`, () => {
        ...
        expect(result.document).toEqual({ ...legacyBase(), schemaVersion: '6.5', layout: form });
    ...
    it(`${schemaVersion} 缺省布局规范化为 report`, () => {
      ...
      expect(result).toMatchObject({ ok: true, document: { schemaVersion: '6.5', layout: 'report' } });
    });
  }
  ...
  it.each(['4.0', '5.5', '6.0', '6.1', '6.2', '6.3', '6.4', '6.6', '7.0', '06.1'])('非支持版本 %s 不得自动升级', (schemaVersion) => {
  ...
  it.each(['5.0', '5.1', '5.2', '5.3', '5.4'])('%s 通过当前结构校验并规范化为 6.1 的唯一写出形状', (schemaVersion) => {
    ...
    expect(result).toMatchObject({
      ok: true,
      document: { ...legacyContent, schemaVersion: '6.5', layout: 'report' }
    });
  });
  it('5.4 的完整站内导航规范化为 6.x 普通 URL 导航，且不改写输入', () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      document: {
        schemaVersion: '6.5', layout: 'report',

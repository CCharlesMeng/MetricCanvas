...
describe('6.0 是 URL 导航的主版本边界', () => {
  it('当前主版本全部原有能力从 0 起算', () => {
    ...
    expect(supportedVersions()).toEqual(['6.0', '6.1']);
    expect(versionPolicy.current).toBe('6.1');
    for(const [id, definition] of Object.entries(pageCapabilities)) expect(definition.minor).toBe(id === 'page-layout' ? 1 : 0);
  ...
  it('旧主版本与未来版本都拒绝，当前版本可读', () => {
    ...
    for(const schemaVersion of ['5.0','5.4','4.0','7.0','6.2','6','6.0.1']) expect(versionErrors({schemaVersion})).toHaveLength(1);

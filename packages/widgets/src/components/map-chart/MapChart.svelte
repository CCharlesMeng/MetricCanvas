<script lang="ts">
  ...
  import { geoRegionName, mapOption } from './options';
  import { ensureBasemap, type BasemapMeta } from './basemap';
   ...
   */
  const rowByGeoName = $derived.by(() => {
    const nameField = resolveField(props.nameField, data).field;
    return new Map(
      data.main.snapshot.rows.map(
        (row) => [geoRegionName(row, nameField, props.nameMap), row] as const
      )
    );
  });

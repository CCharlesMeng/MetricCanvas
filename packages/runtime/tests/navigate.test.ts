...
describe('drillThroughSearch:筛选值 + 点击上下文 → 目标页 URL 查询串', () => {
  ...
  it('点击行缺少占位维度的值时,该 setFilters 项跳过、其余照常', () => {
    const search = drillThroughSearch(
      { page: 'sales-detail', setFilters: { 'f-channel': '$dimension.channel' }, carryFilters: ['f-time'] },
      current,
      { region: '华东' }
    );
    const restored = restore(search);
  ...
  it('setFilters 与 carryFilters 指向同一筛选器时,点击上下文优先', () => {
    const search = drillThroughSearch(
      { page: 'sales-detail', carryFilters: ['f-region'], setFilters: { 'f-region': '$dimension.region' } },
      current,
      { region: '华南' }
    );
  ...
  it('数值型点击值转为字符串写入(URL 载体只有字符串)', () => {
    const search = drillThroughSearch(
      { page: 'sales-detail', setFilters: { 'f-year': '$dimension.year' } },
      new Map(),
      { year: 2026 }
    );

...
内容工具成功表示生成合法产物。报告实际 applied/failed/skipped/unchanged 及呈现调整；全失败或无变化不请求新增修订。只有可信保存回执和精确引用验证后才说已保存，只有真实发布回执才说已发布。同轮续写使用 candidateRef；最终候选由可信程序选定并自动提交草稿。候选/执行记录提供方未接通时明确不可用，生产写路径尚未开放；恢复与发布由集成程序按实际能力处理。

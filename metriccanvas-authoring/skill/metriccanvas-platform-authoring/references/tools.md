...
安装整个Skill目录。启动注入SKILL.md和本文件、八工具实际Schema及可信contextRef；工作流/布局/错误/例子按入口指示注入，或明确提供真实文件读取能力。完整页面创建还须注入 scenarios.md、reading-design.md 及相关场景；重组既有页时按需注入 reading-design.md。无文件读取的部署不能只给链接。完整协议由工具/校验器消费，不要求模型从源码仓加载。

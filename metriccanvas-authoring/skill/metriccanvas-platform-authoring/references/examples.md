...
仅在参数结构需要例子时读取。以下 page_id/token/组件 ID 都必须替换为当前可信上下文值。

## 修改已知组件标题

```json
{
  "baseline_token": "trusted-token",
  "request": {
    "operations": [
...
“这张图怎么配置”：按本轮目标稳定 ID 回答已提供绑定、格式及布局；没有目标配置时请求可信补读，不调用编辑或数据发现。

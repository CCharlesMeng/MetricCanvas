...
溯源（仓库路径）：`metriccanvas-authoring/tool/metriccanvas_authoring/pages/components/component_editing.py`、`domain/page_building.py`、`domain/text_map_building.py`、`domain/container_building.py`；公开闭集测试`metriccanvas-authoring/test-harness/tests/test_content_containers.py`。t08/t09/t10/t11证据位于`docs/archive/authoring-tickets-126/`，它们分别记录本仓静态、实际本地协议与外部待验收边界。

...
class ContentMcpTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_content_server_has_no_compatibility_initialization_or_save_port(self):
        ...
        with patch.object(compatibility, "create_production_server", side_effect=AssertionError("must not initialize")), patch.dict(os.environ, {"METRICCANVAS_TOOL_SURFACE": "invalid-unused-value"}):

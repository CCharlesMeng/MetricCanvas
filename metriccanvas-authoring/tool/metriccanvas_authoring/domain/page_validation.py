...
def validate_page_document(value: Any) -> list[PageContractIssue]:
    ...
    if isinstance(value.get('params'), Mapping) and value['schemaVersion'] != '6.6':
    ...
    param_issues = [*([PageContractIssue("SCHEMA_ERROR", p, m) for p, m in query_reference_issues(value)] if isinstance(value.get("params"), Mapping) else _inline_param_issues(value)), *_param_binding_issues(value), *_page_param_issues(value)]
    if param_issues:
        return param_issues
...
def normalize_page_document(value: Any) -> dict[str, Any]:
    ...
    document["schemaVersion"] = "6.5" if document["schemaVersion"] == "6.5" else "6.6"
...
def _capability_floor_issues(value: Any) -> list[PageContractIssue]:
    ...
    if value['schemaVersion'] not in ('6.5', '6.6'):
        for i, p in enumerate(_param_declarations(value)):
...
def _page_param_issues(value: Mapping[str, Any]) -> list[PageContractIssue]:
    ...
    for index, declaration in enumerate(declarations):
        ...
        if 'value' in declaration and not _matches_param_default({**declaration, 'default': declaration['value']}):
            issues.append(PageContractIssue('SCHEMA_ERROR', declaration.get('path', f'{path}/value'), '实际参数类型不匹配'))
...
def _matches_param_default(declaration: Mapping[str, Any]) -> bool:
    ...
    if declaration['type'] == 'timeRange' and 'path' in declaration:
        return (isinstance(value, dict) and set(value) == {'start', 'end'} and _matches_time_value(value['start'], declaration.get('granularity')) and _matches_time_value(value['end'], declaration.get('granularity')) and value['start'] <= value['end'])

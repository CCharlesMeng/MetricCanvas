    ...
    for group, entries in params.items():
        for i, p in enumerate(entries):
            ...
            if group == "dimensions":
                ...
                if "dim_value_list" in p: item["default"] = p["dim_value_list"]
            elif group == "times":
                item.update(type="timeRange", granularity=p["granularity"])
                if "start" in p or "end" in p: item["default"] = {"start": p.get("start"), "end": p.get("end")}
            else:
                item["type"] = p["type"]
                if "value" in p: item["default"] = p["value"]
            result.append(item)
    return result

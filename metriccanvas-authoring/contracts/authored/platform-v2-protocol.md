...
Contract vectors to run against an internal Adapter: valid new and edit bindings; wrong actor, page, turn, contextRef or revision; confirmation for a different request/version; same-turn query then separate-process compose; concurrent CAS conflict; DQE missing field, null forbidden field and invalid 21st row; saved receipt then handoff rejection; unknown save then changed request. The expected outcomes are respectively the existing `CURRENT_TURN_*`/`CURRENT_PAGE_*`, `ANALYSIS_PLAN_NOT_CONFIRMED`, `WORK_VERSION_CONFLICT`, `SOURCE_ROW_*`, `RELAY_PREVIEW_MISMATCH` and `SAVE_RECONCILIATION_REQUIRED` families. Production loads `adapters.factory:create_adapters()` through the public bootstrap. See [adapter-interface.md](adapter-interface.md); internal implementations remain company-owned.

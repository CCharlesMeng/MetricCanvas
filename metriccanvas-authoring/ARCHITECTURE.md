...
更新：2026-09-16，对齐 main 合入版本 `d5aa4be`（产品实现 `09982cb`、可信评测 runner `e526358`）。本文描述实际源码和能力边界；[目标架构](../docs/plan/2026-09-15-authoring-agent-architecture.md)保留设计解释与目标目录。
...
|---|---|---|
| Platform 创建、当前页修改、配置问答 | [metriccanvas-platform-authoring](./skill/metriccanvas-platform-authoring/SKILL.md) / `metriccanvas-platform-content` | 同一入口，可信上下文门禁；内容工具只产生候选，可信程序提交最终草稿；发布仍须人工精确确认 |
...
决策依据：[ADR-0064](../docs/adr/0064-agent-returns-page-artifact-relay-and-java-own-persistence.md)、[ADR-0078](../docs/adr/0078-dimension-values-templates-and-page-instances.md)、[ADR-0079](../docs/adr/0079-trusted-authoring-turns-gate-content-tools.md)。

## 2. 当前统一创作调用链

```mermaid
flowchart TD
  UI["工作台：锁新输入 → flush → 同步"] --> L["可信提供方：latest + 精确修订 + 身份"]
  ...
  S --> R["冻结操作及载荷 / 持久执行记录"]
  R --> LC["Lifecycle：保存、原操作查询、精确回读、发布"]
  LC --> J["Java 页面资产提供方"]
  R --> RE["authoring_recovery：授权、取消、预算、原操作恢复"]
  RE --> LC
  LC -->|"已核实精确修订"| UI
```

图中本仓程序、端口与本地替身均有实现；真实 latest/身份、Relay 产物分流、Java 原子保存/回读仍待接入验证。独立启动统一 CLI 可列出五工具，未注入可信 current-turn 提供方时返回 `CURRENT_TURN_UNAVAILABLE`，不读取旧 token 代替最新页面。

### 每轮与跨轮

1. 工作台封闭新人工写入口，收敛正在编辑的输入并等待同步；失败保留本地工作。
2. 可信程序取得最新页面及精确修订，注册身份、页、请求/运行/轮次、能力和完整基线；真正新建使用明确空基线。
3. 模型只获得引用与摘要。工具每次重新验证当前可信轮次；补读绑定同一修订，文字明确目标优先于选中状态。
4. 内容工具生成不可变候选，同轮续写保留根基线。独立成功操作可组成合法子集，失败依赖不留下悬空组件。
5. 可信程序只提交选定的最终候选：先冻结操作和载荷，调用生命周期，再精确回读。无变化、全失败或最终回到原内容不新增修订。
6. 回执未知查询原操作；取消、预算耗尽、重启和预览失败不改用新操作重复写入。恢复重新核实当前身份的权限，不复活旧模型编辑轮次。

本轮 hash 校验可信 `documentJson` 的精确 UTF-8 字节，避免 TS/Python 数字重序列化差异；旧产物 canonical hash 与幂等语义保持。结构向量、hash 或引用本身不证明来源或授权。
...
完整候选保留在 `structuredContent.artifactEnvelope` 程序通道，只向模型提供 `modelSummary`。生命周期保存/发布、候选最终选择与执行恢复由可信程序调用，不作为这五个内容工具的直接写入权限。详见唯一模型侧说明[工具与部署约定](./skill/metriccanvas-platform-authoring/references/tools.md)。
...
| 最终一次提交与回读 | [authoring_submission.py](./tool/metriccanvas_authoring/application/authoring_submission.py) |
...
| 保存、发布的既有规则与外部端口 | [lifecycle.py](./tool/metriccanvas_authoring/application/lifecycle.py)、[lifecycle_publish.py](./tool/metriccanvas_authoring/application/lifecycle_publish.py)、[lifecycle_ports.py](./tool/metriccanvas_authoring/application/lifecycle_ports.py) |
...
真实 DeepSeek 评测需用户批准具体目的服务和项目载荷；真实 latest/身份/产物分流、远端原操作查询和精确回读、源规则/刷新、H1–H15 需提供方证据。缺能力保持明确不可用；本地 SQLite、替身与 scripted 轨迹不得算作生产能力或模型通过率。

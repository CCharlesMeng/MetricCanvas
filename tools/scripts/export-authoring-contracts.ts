...
async function buildAuthoringOutputs(): Promise<OutputMap> {
  ...
  outputs.set(
    ...
    json({
      ...
      files: [
        ...await Promise.all(['publication-contract.ts', 'publication-conformance.json', 'authoring-turn.schema.json', 'authoring-turn.conformance.json', 'authoring-turn-contract.ts', 'authoring-turn-protocol.md', 'authoring-turn.bytes.json', 'authoring-candidate.schema.json', 'authoring-candidate-protocol.md', 'authoring-candidate.conformance.json', 'authoring-recovery-protocol.md', 'authoring-ui-recovery-protocol.md', 'source-description.schema.json', 'add-data-component.schema.json', 'authoring-data-mapping-protocol.md'].map(async name => ({file: `authored/${name}`, sha256: sha256(await readFile(path.join(authoringContractRoot, 'authored', name), 'utf8'))}))),

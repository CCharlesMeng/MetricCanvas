...
本文件是 [SKILL.md](../SKILL.md) 工作流的组成部分，进入 Phase C 时完整读取。硬门禁、输出规范 P1–P7、subagent 派发约定与退出门禁以 SKILL.md 为准，本文件不重复。
...
五类攒在同一轮一次问完（P1）：阻断级修不掉、需越界改动、Open Question 待决、`Deferred` 待判、[还原 YELLOW 请求放行](../SKILL.md#还原-yellow-的放行通道)。每条写出：结论编号、为什么修不掉或放不下、要改哪个文件且它为什么在范围外、推测与理由。
...
逐条核对 [退出门禁](../SKILL.md#退出门禁)，全部满足才出 [最终输出](../SKILL.md#最终输出) 的三行索引。有一条不满足而又不属于第 3 节的上报情形，回第 1 节。

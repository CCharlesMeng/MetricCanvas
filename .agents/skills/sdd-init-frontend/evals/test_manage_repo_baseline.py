...
class ManageRepoBaselineTest(unittest.TestCase):
    def test_generated_svelte_kit_does_not_participate_in_freshness(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            repo.mkdir()
            make_repo(repo)
            generated = repo / ".svelte-kit" / "types" / "route_meta_data.json"
            ...
            self.assertIn("| usable | 是 |", status.stdout)
            document = (baseline / "repo-baseline.md").read_text(encoding="utf-8")
            self.assertNotIn(".svelte-kit", document)

    def test_gitignored_story_artifacts_do_not_participate_in_freshness(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            tmp_path = Path(temp_dir)
            repo = tmp_path / "demo"
            baseline = tmp_path / "baseline"
            ...
            artifact = repo / "reference" / "restore-contract.json"
            ...
            self.assertIn("| usable | 是 |", status.stdout)
            document = (baseline / "repo-baseline.md").read_text(encoding="utf-8")
            self.assertNotIn("reference/restore-contract.json", document)

...
| --- | --- | --- |
| `RUN-3` | `dev` | 一条命令起齐本地开发所需的服务。它由 `tools/dev-cli/src/dev.ts` 包着 pnpm 并行启动 platform 与 dqe-sim 两个包的 `dev`,**同时统一注入运行期环境变量**——这是最容易踩的坑,见下面的隐性坑第 1 条 |
...
1. **`RUN-3` 会覆盖你 shell 里的同名变量。** `tools/dev-cli/src/dev.ts` 在 spawn 前把 `PLATFORM_ORIGIN` / `RUNTIME_ORIGIN` / `VITE_DQE_ENDPOINT` / `DQE_ENDPOINT` / `VITE_AI_SUMMARY_ENDPOINT` 全部赋成本地开发值。所以「`.env` 里配了却不生效」在 `RUN-3` 下是预期的;要让 `.env` 生效得走 `RUN-5`。
2. **`RUN-3` 起的是 platform + dqe-sim 两个服务,不含 embed。** embed 没有 `dev` script,它只有 `build` 与浏览器测试。
...
<!-- 覆盖:apps/playground/.env*、apps/platform/.env.example、tools/dev-cli/src/dev.ts,以及全仓 import.meta.env / process.env 引用点(2026-08-28) -->
...
**版本号:3**(质量命令有实质变动时手动加一;它是下游起点命令缓存的键)

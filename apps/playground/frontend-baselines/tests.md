...
| ID | 指路 | 是什么、何时用 | 被引用 |
| --- | --- | --- | --- |
| `TEST-1` | 仓根 `vitest.config.ts` | **vitest 是主测试框架**,单一配置在仓根,`include` 的 glob 覆盖仓根 `tests/` 与 `apps` / `packages`(含 `packages/engine/*`、`packages/server/*` 两个分组目录)/ `tools`。**没有 `environment` 设置**,即全部用例跑在 Node 环境——这决定了下面 `PATTERN-TEST-1`。跑法见 `RUN-16` | 166 个测试文件 |
| `TEST-2` | 各包 `tests/` 目录,文件名 `*.test.ts`;跨交付物的门禁放仓根 `tests/`(`public-api.test.ts`) | 位置约定:测试与 `src/` **平级**,不与被测文件同目录,不用 `__tests__`。**例外**:断言对象跨越多个包时(如四个交付物的公开面快照)放仓根 `tests/`,因为它不属于任何单个包。分布(文件数):`apps/platform` 48、`packages/page` 26、`packages/engine/widgets` 20、`packages/engine/runtime-ui` 16、`packages/engine/runtime` 15、`packages/server/mcp` 10、`apps/playground` 8、`packages/engine/data-gateway` 7、`tools/dqe-sim` 6、`packages/server/persistence-postgres` 3、`packages/server/page-lifecycle` / `packages/server/template-library` / `tools/dev-cli` 各 2、`tools/design-facts` 1 | `TEST-1` 的 glob |
...
| `TEST-4` | `tests/contract.ts`(`packages/server/page-lifecycle`、`packages/server/template-library`),由各领域包的 `memory.contract.test.ts` 与 `packages/server/persistence-postgres/tests/*.contract.test.ts` 分别喂给内存/PostgreSQL 实现 | 跨实现共享的行为契约用例。同一个端口有两份实现时**照这个形状写**,不要给两份实现各写一套断言——任一断言在某一实现下失败就说明行为漂移了 | 每份契约各由 2 个实现测试消费 |
...
| 依据样本 | 全仓 `package.json` 无 jsdom / happy-dom / testing-library 依赖;`vitest.config.ts` 无 `environment`;166 个 vitest 文件里零处 `mount(` / `render(`;`packages/engine/widgets/tests/` 20 个文件测的全是 `options.ts` / `columns.ts` / `rows.ts` / `presentation.ts` / `view-state.ts` 这类纯函数 |
...
| 依据样本 | `packages/server/persistence-postgres/tests/` 下三份测试的 `runIf`;该包的 `@testcontainers/postgresql` 开发依赖 |

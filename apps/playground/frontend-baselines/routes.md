...
<!-- 覆盖:apps/playground/src/routes/、apps/playground/svelte.config.js、apps/playground/vite.config.ts、packages/engine/runtime-ui/src/types.ts、packages/engine/runtime/src/(2026-08-24)。apps/platform/src/routes/ 只做了目录清点,未逐文件确认 -->

| ID | 指路 | 是什么、何时用 | 被引用 |
| --- | --- | --- | --- |
| `ROUTE-1` | `apps/playground/src/routes/`,SvelteKit 文件式路由 | **无显式路由表**:目录结构即路径,`+page.svelte` 是页面,`[pageId]` 是动态段,`(viewer)` 是不进路径的路由组。加宿主界面就是加目录 | SvelteKit 构建期 |
...
| `ROUTE-4` | `routes/+layout.ts`(`ssr = false`、`prerender = false`)+ `svelte.config.js` 的 `adapter-static({ fallback: 'index.html' })` | SPA 装配约定:全客户端渲染、静态托管。**新增路由不要加 `+page.server.ts` 或 load 函数**,那会与静态 SPA 假设冲突 | 全部 playground 路由 |
| `ROUTE-5` | `RuntimeNavigation`(`packages/engine/runtime-ui/src/types.ts`)+ playground 侧适配实现(`(viewer)/pages/[pageId]/+page.svelte` 内的 `navigation` 对象) | **跳转端口**:统一运行时只上抛跳转意图(`href` / `replaceSearch` / `navigate`),宿主决定怎么跳。这是本文件最值钱的一条,见 `PATTERN-ROUTE-1` | 统一运行时全部跳转点;playground、embed、platform 三个宿主各自实现 |
| `ROUTE-6` | `pageHref` / `rememberPageReturn` / `pageReturnOf`(`apps/playground/src/lib/page-return.ts`) | 参考宿主的回跳记录:运行时只上抛来源页与来源查询串,宿主自己记、自己执行返回;深链接无来源时隐藏回退入口。**这是宿主职责的样板,不是运行时能力** | 看板页面查看器路由 |
...
| 依据样本 | `RuntimeSurface.svelte` 的 `navigate` / `textLink` 只调 `navigation?.navigate` 与 `navigation?.href`,端口缺席时退化为锚点占位;`$app/navigation` 的导入在 playground 侧只出现在看板页面查看器一个文件 |
...
| 依据样本 | `apps/playground/src/lib/page-repository.ts` 用 `import.meta.glob('$pages/*.json')` 枚举;`(viewer)/+page.svelte` 直接渲染 `pageRepository.list()`;10 份页面文档没有任何一份在路由里出现过 |
...
| 项 | 内容 |
| --- | --- |
| 规则 | playground 侧**没有**任何在进入路由前能中断或改写跳转的机制:无 `hooks.client.ts`、无 load 守卫、无页面级鉴权判断。所有路由都是公开的。计划期据此**不要**挂守卫,检视**不得**判「需要登录的路由漏了守卫」 |
| 依据清单 | `ROUTE-1`、`ROUTE-4` |
| 依据样本 | `apps/playground/src` 下无 `hooks.*` 文件(全仓只有 `apps/platform/src/hooks.server.ts`);playground 三个路由页面无鉴权分支;数据请求靠 `credentials: 'same-origin'` 的 cookie,见 `PATTERN-API-4` |

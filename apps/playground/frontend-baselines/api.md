...
<!-- 覆盖:全仓源码的底层 HTTP 调用点(fetch / XMLHttpRequest / EventSource / axios 全量 grep),其中 packages/*/src/ 与 apps/playground/src/ 逐文件确认;apps/platform/src/ 只确认了调用点位置与用途,未逐文件展开(2026-08-24) -->
...
| `API-2` | `createDqeGateway`(`packages/engine/data-gateway/src/dqe.ts`,经包出口导出) | `API-1` 的**直连实现**:浏览器直接打远程查询端点。批量信封、并发额度、超时、取消传递、结算幂等、错误分类、诊断落点全部隐藏在里面。playground 与 embed 用它;改取数行为改这里,不要在调用侧绕 | playground 与 embed 的服务装配处 |
| `API-9` | `createInjectedDqeGateway`（`packages/application-runtime`，页面试验场与平台共用） | `API-2` 的第一方装配：每次请求现读注入端点与三个身份头；平台与页面试验场共用 DQE 适配器，不再存在第二个浏览器代理实现 | 平台问数、页面搭建工作台与页面试验场服务装配 |
...
| `API-4` | `PageRepository`(`packages/engine/runtime/src/ports.ts`),实现 `createStaticPageRepository`(`apps/playground/src/lib/`) | 页面文档的取件端口。页面试验场只读静态文件(`STRUCT-7`)，旧平台 API 实现已移除。**`load` 返回 `unknown`**——加载与校验是两步,拿到的是不可信文档 | 目录页与查看器路由 |
...
| `API-8` | `tools/dqe-sim`(`sim:dqe` / `dev:dqe` script,出口 `src/server.ts` 与 `src/execute.ts`) | 本仓的 mock 约定:一个仿真 DQE 服务,`RUN-3` 会自动起它；页面试验场按需联调并通过共享运行配置模块取端点。**要 mock 取数就起它,不要在测试里 stub `fetch`** | `RUN-3`、`packages/engine/data-gateway` 的开发依赖 |
...
| 依据样本 | 全仓底层 HTTP 调用点共 25 处:`API-2` 内 2 处(取数与候选值)、`API-9` 内 2 处、`API-4` 的平台实现 2 处、`API-5` 1 处、platform 自身 `/api/*` 与模型调用 18 处;`packages/engine/runtime`、`packages/engine/runtime-ui`(除 `API-5`)、`packages/engine/widgets` 内零命中 |
| 违例判定 | 出现新的取数路径而没经 `API-1`(即 `fetch(` 直连查询端点),或在 `packages/engine/runtime*` / `packages/engine/widgets` 内新增底层 HTTP 调用 |

#### `PATTERN-API-2` · 网关由宿主注入,运行时只依赖端口

| 项 | 内容 |
| --- | --- |
| 规则 | 适配器(`API-2` / `API-9`)的构造只发生在宿主装配处;`packages/engine/runtime`、`packages/engine/runtime-ui`、`packages/engine/widgets` 不得导入 `@metriccanvas/engine/dqe`,也不得出现端点字面量。**第二形态(嵌入)下网关由第三方宿主经 `mount` 传入**,所以在运行时包里搜不到出口是设计结果,不是缺失 |
| 依据清单 | `API-1`、`API-2`、`API-9`、`STRUCT-3`、`STRUCT-6` |
| 依据样本 | `apps/playground/src/lib/services.ts` 是 playground 唯一构造点;`packages/embed/src/index.ts` 把 `dataGateway` 作为挂载选项透传;`packages/engine/runtime/src/ports.ts` 的注释写明「适配器在 data-gateway,应用壳注入」;`packages/engine/runtime*/package.json` 的 dependencies 不含 data-gateway |
| 违例判定 | `packages/engine/runtime*` 或 `packages/engine/widgets` 的 `package.json` 或源码出现 `@metriccanvas/engine/dqe`;或运行时内出现 `/rest/...` 之类端点字面量 |

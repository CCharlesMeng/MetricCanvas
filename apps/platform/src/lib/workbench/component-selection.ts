...
/**
 * 人工组件选择（创作期）：以组件能力目录 `componentCatalog` 为唯一硬闸来源，
 * 由取数单元真实执行后的结果形状推荐组件。硬闸不满足者一律 `ok: false`
 * 并携带原因；允许范围内按分析意图排序，首位标为推荐（ADR-0037）。
 * 纯函数：不依赖浏览器、不依赖统一运行时、不做 IO。
 */

/** 与局部构造 switch 同步；目录新增类型时先拒绝，避免出现可选但不可构造的项。 */
const constructibleTypes = new Set<ComponentCatalogEntry['type']>([
  'metricCard', 'barChart', 'lineChart', 'table', 'pieChart',
  'rankingCard', 'rankingDetailCard', 'gauge', 'keyValuePanel', 'categoryBreakdown'
]);
...
function evaluateCandidate(
  ...
  if (!constructibleTypes.has(entry.type)) reasons.push('页面搭建工作台不支持该组件的局部构造');
  return {
    type: entry.type,
    label: entry.label,

...
const entry=`import {mount} from 'svelte';import Harness from '/__harness.svelte';import {prepareExecution} from '@metriccanvas/engine';
const fixture=${JSON.stringify(fixture)};
window.pending={};window.signals={};window.calls=[];
...
window.resolveRead=id=>{const document=structuredClone(fixture.response.document);document.sections[0].components[0].props.title='READ-'+id;for(const [key,ds] of Object.entries(document.dataSources)){ds.source.initial={capturedAt:'2026-09-14T00:00:00Z',rows:fixture.response.dataSources[key].rows};}window.pending['read:'+id]({pageId:'t04-example',revisionId:id,resourceId:'metadata-a',revisionNumber:1,document,baseRevisionId:null,contentHash:'fixture',dataContextVersion:null,createdBy:'fixture',createdAt:'2026-09-14'});};
window.resolveExecution=(id,wrong=false)=>{const request=structuredClone(fixture.request);request.target.ref.revisionId=wrong?'wrong':id;request.operationId='execute-'+id;const response=structuredClone(fixture.response);response.target=request.target;response.operationId=request.operationId;response.document.sections[0].components[0].props.title='EXEC-'+id;window.pending['execute:'+id](prepareExecution(request,response));};
...
try{
 ...
 console.log('RevisionPreview: exact read→execution, abort/late isolation, mismatched target, unmount and no-executor fallback passed');

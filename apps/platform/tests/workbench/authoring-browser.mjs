...
try {
  ...
  await expect(manual.getByLabel('精确修订预览 r1').getByText('原标题')).toBeVisible();
  await manual.getByRole('button', { name: '精确修订预览' }).click();
  await inspector.getByRole('button', { name: /原标题/ }).click();
  await inspector.getByLabel('组件标题').fill('本地保护的新标题');
  await inspector.getByLabel('组件标题').press('Tab');
  await expect(manual.getByText(/已在浏览器保护，待同步 1 个操作/)).toBeVisible();
  await expect(manual.getByText(/服务尚未确认稳定幂等保存/)).toBeVisible();
  expect(writes).toBe(0);
  await manual.screenshot({ path: '/private/tmp/metriccanvas-s1-evidence/t13-unavailable.png' });
  await manual.goto(`${root}/manage/pages/manual-page`);
  await expect(manual.getByText(/尚未开放历史修订读取/)).toBeVisible();
  await manual.close();
  console.log('T02/T13 browser PASS: legacy open, fixed preview/history limits retained; auto edit is locally protected and never uses unstable legacy PUT.');

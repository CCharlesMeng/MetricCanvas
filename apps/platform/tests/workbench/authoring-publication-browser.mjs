...
async function preview(){await page.getByRole('button',{name:'预览当前候选'}).click();await expect(page.getByLabel('候选执行预览')).toBeVisible();await expect(page.getByText('DATA_GATEWAY_REQUIRED')).toHaveCount(0);await expect(page.getByLabel('候选执行预览').getByText('Regional sales',{exact:true})).toBeVisible();}
try{
 ...
 await preview();await acknowledged.check();await mkdir('/private/tmp/metriccanvas-s1-evidence',{recursive:true});await page.screenshot({path:'/private/tmp/metriccanvas-s1-evidence/t19-review.png',fullPage:true});await publish.click();await expect(page.getByTestId('published-reference')).toContainText('immutable-template');
 ...
 await expect(workbench.getByTestId('publication-status')).toContainText('CAPABILITY_UNAVAILABLE');expect(writes).toBe(0);await workbench.getByRole('button',{name:'发布评审',exact:true}).click();
 await expect(workbench.getByRole('main',{name:'页面画布'}).getByText('保留工作副本')).toBeVisible();await workbench.getByRole('complementary',{name:'检查器'}).getByRole('button',{name:/保留工作副本/}).click();await expect(workbench.getByLabel('组件标题')).toBeEnabled();await workbench.close();
 ...
 expect(errors).toEqual([]);console.log('T19 UI PASS: corrections/preview/explicit human confirmation, expiry/lease/identity boundaries, stale source, blocking, lost receipt lookup, immutable publication.'+(process.env.S1_PRODUCTION_URL?' Production gate PASS.':''));

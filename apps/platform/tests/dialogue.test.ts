...
import { panguDialogueAdapter } from '../src/lib/dialogue/runtime';
...
describe('dialogue seam', () => {
  it('mounts a placeholder and destroys only its own node, repeatedly and independently', async () => {
    ...
    const destroy = await panguDialogueAdapter.mount(element);
    const destroyOther = await panguDialogueAdapter.mount(other.element);
  ...
  it('delivers an immutable exact draft reference across the event boundary', () => {
    ...
    const detail = { draftId: 'draft/revision-17', pageDocument: {} };
  ...
  it.each([null, undefined, {}, { draftId: 42 }, { draftId: '' }, { draftId: '  ' }])(

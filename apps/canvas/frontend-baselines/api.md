...
| `API-9` | `createInjectedDqeGateway`（`packages/application-runtime`，Canvas 与平台共用） | `API-2` 的第一方装配：每次请求现读注入端点与三个身份头；平台与 Canvas 共用 DQE 适配器，不再存在第二个浏览器代理实现 | 平台问数、页面搭建工作台与 Canvas 服务装配 |
...
| 依据样本 | `apps/canvas/src/lib/services.ts` 是 canvas 唯一构造点;`packages/embed/src/index.ts` 把 `dataGateway` 作为挂载选项透传;`packages/engine/runtime/src/ports.ts` 的注释写明「适配器在 data-gateway,应用壳注入」;`packages/engine/runtime*/package.json` 的 dependencies 不含 data-gateway |

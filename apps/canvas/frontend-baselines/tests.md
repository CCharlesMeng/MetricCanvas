...
<!-- 覆盖:仓根 vitest.config.ts、apps/*/tests/、packages/*/tests/、tools/*/tests/(2026-08-24) -->

| ID | 指路 | 是什么、何时用 | 被引用 |
| --- | --- | --- | --- |
| `TEST-1` | 仓根 `vitest.config.ts` | **vitest 是主测试框架**,单一配置在仓根,`include` 三段 glob 覆盖 `apps` / `packages` / `tools`。**没有 `environment` 设置**,即全部用例跑在 Node 环境——这决定了下面 `PATTERN-TEST-1`。跑法见 `RUN-16` | 146 个测试文件 |
| `TEST-2` | 各包 `tests/` 目录,文件名 `*.test.ts` | 位置约定:测试与 `src/` **平级**,不与被测文件同目录,不用 `__tests__`。分布(文件数):`packages/page` 24、`packages/runtime` 15、`packages/widgets` 13、`packages/runtime-ui` 12、`packages/mcp` 9、`apps/canvas` 8、`packages/data-gateway` 7、`tools/dqe-sim` 5、`packages/template-library` 4、`packages/page-lifecycle` 3,`apps/platform` 44 | `TEST-1` 的 glob |
...
| `TEST-4` | `tests/contract.ts`(`packages/page-lifecycle`、`packages/template-library`),由 `memory.contract.test.ts` 与 `postgres.contract.test.ts` 分别喂给两份实现 | 跨实现共享的行为契约用例。同一个端口有两份实现时**照这个形状写**,不要给两份实现各写一套断言——任一断言在某一实现下失败就说明行为漂移了 | 各 2 个测试文件 |
...
| 依据样本 | 全仓 `package.json` 无 jsdom / happy-dom / testing-library 依赖;`vitest.config.ts` 无 `environment`;146 个 vitest 文件里零处 `mount(` / `render(`;`packages/widgets/tests/` 13 个文件测的全是 `options.ts` / `columns.ts` / `rows.ts` / `presentation.ts` / `view-state.ts` 这类纯函数 |
...
| 依据样本 | `packages/page-lifecycle/tests/postgres.contract.test.ts` 与 `packages/template-library/tests/postgres.test.ts` 的 `runIf`;两包的 `@testcontainers/postgresql` 开发依赖 |

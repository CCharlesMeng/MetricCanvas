...
<!-- 覆盖:仓根 package.json / pnpm-workspace.yaml / pnpm-lock.yaml / compose.yaml、apps/*/package.json、packages/*/package.json、tools/*/package.json(2026-08-24) -->

| ID | 指路 | 是什么、何时用 | 被引用 |
| --- | --- | --- | --- |
| `RUN-1` | 仓根 `pnpm-lock.yaml` + `pnpm-workspace.yaml` | 包管理器锁定为 **pnpm**,工作区三段 `packages/*` / `apps/*` / `tools/*`,共 13 个工作区包 + 仓根。装依赖只在仓根跑一次,不要进子包装 | 全部工作区包 |
...
| `RUN-6` | `db:up` / `db:down` + 仓根 `compose.yaml` | 起 / 停本地 Postgres。只有页面生命周期与模板库的 Postgres 实现需要它,查看器主链路不需要 |
...
3. `RUN-6` 起的 Postgres 只服务 `packages/page-lifecycle` 与 `packages/template-library`;它们的 Postgres 测试还要额外的开关,见 `PATTERN-TEST-3`。
4. 质量命令一律**在仓根跑**。`test` 与 `validate` 都定义在仓根且依赖仓根路径(`vitest.config.ts` 的 glob、页面资产目录),进子包跑会找不到用例或页面。

## 环境变量

<!-- 覆盖:apps/canvas/.env*、apps/platform/.env.example、tools/dev-cli/src/dev.ts,以及全仓 import.meta.env / process.env 引用点(2026-08-24) -->
...
| `RUN-13` | `AGENT_MODEL_PROVIDER`、`DEEPSEEK_API_KEY`、`DEEPSEEK_MODEL`、`DEEPSEEK_BASE_URL` | platform 服务端 | **`DEEPSEEK_API_KEY` 是** | 键名在模板里 |
...
<!-- 覆盖:仓根 package.json scripts、13 个工作区包的 check script、packages/embed/playwright.config.ts、.github/(2026-08-24) -->

**版本号:1**(质量命令有实质变动时手动加一;它是下游起点命令缓存的键)
...
| `RUN-18` | `check` | 全部 13 个工作区包的类型检查(`tsc --noEmit` + `svelte-check`) | 无。首次会在 app 目录生成 `.svelte-kit`(已 gitignore) |
...
| 依据清单 | `RUN-16`~`RUN-20` |
| 依据样本 | 仓根 `package.json` 的 scripts 段;13 个子包的 `check` script 形态一致;`.github/` 下无额外质量步骤 |
